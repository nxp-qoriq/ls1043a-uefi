...
typedef struct {
  ...
  UINT8   SectorsPerCluster;	//2^n
  UINT16  ReservedSectors;		//atleast 1
  UINT8   NumFats;			//typically 2
  UINT16  RootEntries;          // < FAT32, root dir is fixed size, 1 to 512
  UINT16  Sectors;			//no of small sectors
  UINT8   Media;			/* Byte   Capacity   Media Size and Type
...
typedef struct {
  ...
  CHAR8 Id[4];
  CHAR8 FatLabel[11];
  CHAR8 SystemId[8];		//FAT12 OR FAT16
} FAT_BOOT_SECTOR_EXT;
